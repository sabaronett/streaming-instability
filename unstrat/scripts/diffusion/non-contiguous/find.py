...
# NOTE: This version of par_disp() accounts for outputs with non-contiguous
# (dropped) particle IDs by using athena_read.particles(indexing=False) to
# track only the IDs (`ids`) present in the last output.
#
...
def par_disp(datadir='./dat', save_to=None, athinput=None, nout=3):
    ...
    time, pdata = athena_read.particles(str(outputs[-1]), indexing=False)
    ids = pdata['id']
    npar = pdata.size
    nt = len(outputs)
