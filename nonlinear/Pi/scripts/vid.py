...
# Set passed arguments, cmap log min/max, dpi
vmin = 1e-1 # CPDD > 70%
vmax = 1e1  # CPDD ~ 1%
dpi = 450   # 2160p default
run = 'AB'

# Collect Athena++ inputs & outputs
athinput = athena_read.athinput('../athinput.si')
if athinput['problem']['epsilon'] == 0.2: run = 'BA'
res = athinput['mesh']['nx1']              # box mesh resolution
...
ax.set_aspect('equal')
if run == 'AB': ax.set_title('$t={:.2f}$ / $T$'.format(times[0]))
else: ax.set_title('$t={:.1f}$ / $T$'.format(times[0]))
ax.set_xlabel('$x$ / $H$')
ax.set_ylabel('$z$ / $H$')
