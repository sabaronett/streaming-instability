...
# Find min., max., avg., std. dev. of each ordered rhop over saturated state
min_rhops = np.amin(rhops, axis=0)
max_rhops = np.amax(rhops, axis=0)
avg_rhops = np.average(rhops, axis=0)
std_rhops = np.std(rhops, axis=0)
...
ax.fill_between(np9_rhops, np9_cdf, np9_cdf-.1, alpha=0.2, label='Min./Max.')
ax.loglog(np16_rhops, np16_cdf, label='Average')
ax.fill_between(np16_rhops, np16_cdf-.01, np16_cdf+.0001, alpha=0.4, label=r'$\pm\sigma$')
...
plt.savefig('../plots/CPDD2_np{:.0f}.pdf'.format(Np), bbox_inches='tight',
            pad_inches=0.01)
writetxt(cut_rhops, cdf, '../plots/CPDD_np{:.0f}_min.txt'.format(Np))
writetxt(cut_rhops, cdf, '../plots/CPDD_np{:.0f}_max.txt'.format(Np))
writetxt(cut_rhops, cdf, '../plots/CPDD_np{:.0f}_avg.txt'.format(Np))
writetxt(cut_rhops, cdf, '../plots/CPDD_np{:.0f}_std.txt'.format(Np))
